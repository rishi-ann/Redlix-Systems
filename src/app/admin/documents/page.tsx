                        ...
                        documents.map((doc) => (
                            <div key={doc.id} className="group p-6 bg-white dark:bg-zinc-900/40 border border-zinc-100 dark:border-zinc-800 rounded-2xl hover:border-emerald-200 dark:hover:border-emerald-900/50 transition-all shadow-sm">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <span className="text-[9px] text-zinc-400 font-mono">{new Date(doc.createdAt).toLocaleDateString()}</span>
                                </div>
                                ...
                                <div className="flex items-center gap-2 mb-3">
                                    ...
                                    {doc.client.developers && doc.client.developers.length > 0 && (
                                        <span className="text-[9px] text-zinc-400 px-1.5 py-0.5 bg-zinc-100 dark:bg-zinc-800 rounded-full">
                                            Devs: {doc.client.developers.map((d: any) => d.email.split('@')[0]).join(', ')}
                                        </span>
                                    )}
                                </div>
